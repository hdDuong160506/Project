...
def api_products():
    ...
    if price_filter:
        ...
        if price_filter in ranges:
            ...
            for r in results:
                r["store"] = [s for s in r["store"] 
                            if s.get("ps_min_price_store") is not None and s.get("ps_max_price_store") is not None and 
                            ((low <= s["ps_min_price_store"] <= high) or (low <= s["ps_max_price_store"] <= high) or
                            (s["ps_min_price_store"] <= low and s["ps_max_price_store"] >= high))]
            results = [r for r in results if r["store"]]
