...
def api_products():
    ...
    # [MỚI] Lấy tham số product_id để lọc cho trang chi tiết
    ...
    # Gọi hàm lấy dữ liệu thô từ database
    rows = fetch_data_from_database()
    
    # --- Logic Tìm kiếm và Lọc Product ID ---
    
    if search_text:
        # Nếu có search_text, vẫn chạy logic tìm kiếm hiện tại (có thể lọc theo tên/địa điểm)
        results = search_location(search_text, rows, user_lat, user_lon)
    else:
        # Nếu không có search_text, xây dựng map sản phẩm
        product_map = build_product_map(rows, user_lat, user_lon)
        results = list(product_map.values())
        
    # [MỚI] Logic Lọc theo Product ID: Chỉ áp dụng khi không có search_text
    if product_id_filter:
        try:
            target_id = int(product_id_filter)
            # Lọc danh sách results, chỉ giữ lại sản phẩm có product_id trùng khớp
            results = [r for r in results if r["product"].get("product_id") == target_id]
        except ValueError:
            # Xử lý nếu product_id_filter không phải là số
            pass


    # Logic lọc khoảng cách (giữ nguyên)
    if distance_filter:
    ...
    # Format dữ liệu JSON trả về (giữ nguyên)
    products = []
    for p in results:
