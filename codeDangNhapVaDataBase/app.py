...
def register():
    ...
    pwd = data.get('pwd') # Lấy 'pwd' thay vì 'password'
    ...
    new_user = User(
        name=name, 
        email=email, 
        pwd=hashed_password.decode('utf-8') # Lưu vào cột 'pwd'
    )
...
def login():
    ...
    if user and bcrypt.checkpw(pwd.encode('utf-8'), user.pwd.encode('utf-8')):
