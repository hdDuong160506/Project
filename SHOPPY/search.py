...
from services.search_service import search_location

app = Flask(__name__, static_folder='static', static_url_path='')

# ✅ Route cho trang chính (web.html)
@app.route('/')
def home():
...
def api_products():
    ...
    results = search_location(search_text, rows, user_lat, user_lon) if search_text else []

    # Nếu không có search_text, trả về tất cả sản phẩm
    if not search_text:
        from services.search_service import build_product_map
